...
> [!important] Edit note bodies with Read/Edit/Grep — `obsidian-cli` complements it, it does not replace it
> As of Obsidian 1.13.7 the CLI has **no `edit` command** — only append / prepend /
> overwrite — so note bodies stay with Read/Edit/Grep.
>
...
> [!warning] Inside a table cell, the display-text pipe MUST be escaped as `\|`
> Markdown reads the bare `|` as a column separator: the link is truncated mid-way (renders as garbage like `([[#3.2 Routing rules`) and the row gains a spurious column.
>
> ```markdown
> | Change | See |
> |---|---|
> | Routing rules updated | （[[#3.2 Routing rules\|see 3.2]]） |
> ```
>
...
Derived from [`kepano/obsidian-skills`](https://github.com/kepano/obsidian-skills/tree/main/skills/obsidian-markdown) (MIT, Copyright (c) 2026 Steph Ango). Modified in this repo; the upstream licence is included as `LICENSE`.
