...
This repository carries the **prompt-and-code half** of the skill: `SKILL.md`, `references/`,
`workflows/`, `scripts/`, `.env.example`, `requirements.txt`.

`templates/` is **not distributed** — neither here nor on the web. It holds three things that
cannot go into a public repository as they are:

- `templates/icons/` — 11,600+ SVGs vendored from third-party libraries (Tabler, Phosphor,
  Simple Icons and others) without their licence notices.
- `templates/layouts/` — deck layouts, several of them replicated from specific organisations'
  brand decks, including logo bitmaps.
- `templates/charts/` — chart templates that reference the same design specs.

Every reference to `templates/…` below therefore describes files you supply yourself. Build the
directory as `templates/{icons,layouts,charts}/` following the paths named in this file and in
`workflows/create-template.md`, which documents the `design_spec.md` + `layouts_index.json`
format a layout must follow.

https://www.kzgai.cloud/skills/ppt-master hosts a docs-only package (`SKILL.md`, `references/`,
`workflows/`, `.env.example`, `requirements.txt`) — the site caps a skill at 64 files / 512 KB
and accepts text only, so `scripts/` is not there either. Read it on the site, install it here.
